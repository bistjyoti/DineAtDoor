...
const DonateFood = ({ restaurantId = "65f1a2b3c4d5e6f7a8b9c0d1" }) => { 
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            // URL check kar lena, backend 4000 par hi hai na?
            const response = await axios.post('http://localhost:4000/api/donations/donate', {
                ...foodData,
                restaurantId 
            });
            ...
            // Form reset
            setFoodData({ foodItems: '', quantity: '', expiryTime: '' });
        } catch (error) {
            console.error("Donation Error:", error);
            const errorMsg = error.response?.data?.error || "Connection fail! Juhiee's console dekho.";
            alert(errorMsg);
        }
    };
